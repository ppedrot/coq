...
let get_relevance = function
  | LocalAssum (x,_) | LocalDef (x,_,_) -> x.binder_relevance
...
let rec relevance_of_fterm env extra lft f =
  ...
  | FLambda (len, tys, bdy, e) ->
    (* TODO check order *)
    let extra = List.rev_append (List.rev_map (fun (x,_) -> x.binder_relevance) tys) extra in
  ...
  | FLetIn (x, _, _, bdy, e) ->
    relevance_of_term_extra env (x.binder_relevance :: extra) (Esubst.el_lift lft) (Esubst.subs_lift e) bdy
...
and relevance_of_term_extra env extra lft subs c =
  ...
  | Prod (x, _, codom) ->
    relevance_of_term_extra env (x.binder_relevance::extra) (Esubst.el_lift lft) (Esubst.subs_lift subs) codom
  | Lambda (x, _, bdy) ->
    relevance_of_term_extra env (x.binder_relevance::extra) (Esubst.el_lift lft) (Esubst.subs_lift subs) bdy
  | LetIn (x, _, _, bdy) ->
    relevance_of_term_extra env (x.binder_relevance::extra) (Esubst.el_lift lft) (Esubst.subs_lift subs) bdy
