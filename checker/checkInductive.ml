  ...
  let rec chk env rctx1 rctx2 =
    ...
    match rctx1, rctx2 with
        (LocalAssum (_,ty1) as d1)::rctx1', LocalAssum (_,ty2)::rctx2' ->
          Reduction.conv ~typed:true Reduction.CONV env ty1 ty2;
          chk (push_rel d1 env) rctx1' rctx2'
      | (LocalDef (_,bd1,ty1) as d1)::rctx1', LocalDef (_,bd2,ty2)::rctx2' ->
          Reduction.conv ~typed:true Reduction.CONV env ty1 ty2;
          Reduction.conv ~typed:true Reduction.CONV env bd1 bd2;
          chk (push_rel d1 env) rctx1' rctx2'
      | [],_ -> ()
...
let typecheck_arity env params inds =
  ...
  let check_arity arctxt = function
    | RegularArity mar ->
        ...
        Reduction.conv ~typed:true Reduction.CONV env (Term.it_mkProd_or_LetIn (Constr.mkSort mar.mind_sort) arctxt) ar;
        ar
    | TemplateArity par ->
...
let typecheck_one_inductive env params mip =
  ...
  Array.iter2 (Reduction.conv ~typed:true Reduction.CONV env) mip.mind_nf_lc mip.mind_user_lc;
  (* mind_consnrealdecls *)
