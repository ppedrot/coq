...
let eauto_unif_flags = auto_flags_of_state Conv_oracle.empty
		   ...
		   List.map (fun gl ->
		     ...
		     let hyps' = pf_hyps gls in
		       if hyps' == hyps then List.hd s.localdb
                       else make_local_hint_db (pf_env gls) (project gls) ~ts:Conv_oracle.empty true s.local_lemmas)
...
let e_search_auto debug (in_depth,p) lems db_list gl =
  let local_db = make_local_hint_db (pf_env gl) (project gl) ~ts:Conv_oracle.empty true lems in
