...
let constr_val_discr_st sigma st t =
  let c, l = decomp sigma t in
    match EConstr.kind sigma c with
    | Const (c,u) -> if Conv_oracle.is_transparent_constant st c then Everything else Label(GRLabel (ConstRef c),l)
    | Ind (ind_sp,u) -> Label(GRLabel (IndRef ind_sp),l)
    | Construct (cstr_sp,u) -> Label(GRLabel (ConstructRef cstr_sp),l)
    | Var id when not (Conv_oracle.is_transparent_variable st id) -> Label(GRLabel (VarRef id),l)
...
let constr_pat_discr_st st t =
  match decomp_pat t with
  | PRef ((IndRef _) as ref), args
  | PRef ((ConstructRef _ ) as ref), args -> Some (GRLabel ref,args)
  | PRef ((VarRef v) as ref), args when not (Conv_oracle.is_transparent_variable st v) ->
      Some(GRLabel ref,args)
  | PVar v, args when not (Conv_oracle.is_transparent_variable st v) ->
      Some(GRLabel (VarRef v),args)
  | PRef ((ConstRef c) as ref), args when not (Conv_oracle.is_transparent_constant st c) ->
      Some (GRLabel ref, args)
