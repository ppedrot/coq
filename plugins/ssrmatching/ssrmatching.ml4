...
let ungen_upat lhs (c, sigma, uc, t) u =
  ...
  | Lambda _ -> KpatLam
  | _ -> KpatRigid in
  c, sigma, uc, {u with up_k = k; up_FO = lhs; up_f = f; up_a = a; up_t = EConstr.Unsafe.to_constr t}
...
type find_P = 
  ...
  k:subst ->
     constr
type conclude = unit ->
  constr * ssrdir * (bool * Evd.evar_map * UState.t * constr)
...
((fun () ->
  let c, sigma, uc, ({up_f = pf; up_a = pa} as u) =
