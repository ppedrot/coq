...
let pf_mkprod gl c ?(name=constr_name (project gl) c) cl =
  ...
  if name <> Anonymous || EConstr.Vars.noccurn (project gl) 1 cl then gl, EConstr.mkProd (make_annot name Sorts.Relevant, t, cl) else
  gl, EConstr.mkProd (make_annot (Name (pf_type_id gl t)) Sorts.Relevant, t, cl)
...
let pf_interp_gen_aux gl to_ind ((oclr, occ), t) =
  ...
  if not(occur_existential sigma c) then
    ...
    false, pat, EConstr.mkProd (make_annot (constr_name (project gl) c) Sorts.Relevant, pty, Tacmach.pf_concl gl), p, clr,ucst,gl
