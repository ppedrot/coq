		...
		ar2.(j) <- let cc = (match nb_cstr_args with
   		  ...
   		  in
                    ...
                    (constrsj.(j).cs_args)
                        ...
                        mkLambda (RelDecl.get_annot decl, RelDecl.get_type decl, a)) (Lazy.force ff) (constrsj.(j).cs_args) )
	    done;
    ...
    let x = next_ident_away (Id.of_string "x") avoid and
      ...
      let eqs_typ = List.map (fun (s,_,_,_) ->
        mkProd(make_annot Anonymous Sorts.Relevant,mkVar s,
               mkProd(make_annot Anonymous Sorts.Relevant,mkVar s,(Lazy.force bb)))
          ) list_id in
