...
let iter_with_binders g f n c = match kind c with
  ...
  | Case (_,p,is,c,bl) -> f n p; Option.iter (f n) is; f n c; Array.Fun1.iter f n bl
  | Proj (p,c) -> f n c
  | Fix (_,(_,tl,bl)) ->
...
let fold_map f accu c = match kind c with
  ...
  | Case (ci,p,is,b,bl) ->
      ...
      let accu, is' = Option.Smart.fold_map f accu is in
...
let compare_head_gen_leq_with kind1 kind2 leq_universes leq_sorts eq leq nargs t1 t2 =
  ...
  | Case (_,p1,is1,c1,bl1), Case (_,p2,is2,c2,bl2) ->
    eq 0 p1 p2 && Option.equal (eq 0) is1 is2 && eq 0 c1 c2 && Array.equal (eq 0) bl1 bl2
...
let constr_ord_int f t1 t2 =
  ...
  match kind t1, kind t2 with
    ...
    | Case (_,p1,is1,c1,bl1), Case (_,p2,is2,c2,bl2) ->
      let c = f p1 p2 in
      if Int.equal c 0 then let c = Option.compare f is1 is2 in
...
module HashsetTermOption =
  Hashset.Make(struct type t = constr option let eq = Option.equal hasheq end)
...
let term_option_table = HashsetTermOption.create 1999
...
let hashcons (sh_sort,sh_ci,sh_construct,sh_ind,sh_con,sh_na,sh_id) =
	...
	(Construct (sh_construct c, u'),
	 combinesmall 11 (combine (constructor_syntactic_hash c) hu))
      | Case (ci,p,is,c,bl) ->
        ...
        let is, his = hash_term_option is in
        let c, hc = sh_rec c in
  ...
  and hash_term_option = function
    | Some t ->
      let t, h = hash_term t in
      let h = h+1 in
      let t = Some t in
      (HashsetTermOption.repr h t term_option_table, h)
    | None ->
      None, 0
...
let rec hash t =
  match kind t with
    ...
    | Case (_ , p, is, c, bl) ->
      combinesmall 12 (combine4 (hash c) (hash p) (Option.hash hash is) (hash_term_array bl))
    | Fix (ln ,(_, tl, bl)) ->
