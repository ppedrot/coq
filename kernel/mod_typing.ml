	...
	| _ -> error_not_a_constant lab
	 ...
	 as long as they have the right type *)
        ...
        | Monomorphic_const _, None ->
          let c',cst = match cb.const_body with
            | Undef _ | OpaqueDef _ ->
              ...
              let cst' = Reduction.infer_conv ~typed:true Reduction.CUMUL env' (Environ.universes env')
                  j.uj_type typ in
              j.uj_val, cst'
            | Def cs ->
              let c' = Mod_subst.force_constr cs in
              c, Reduction.infer_conv ~typed:true Reduction.CONV env' (Environ.universes env') c c'
          in
	  ...
	  let cst = match cb.const_body with
	    ...
	    | Def cs ->
	       let c' = Mod_subst.force_constr cs in
               let cst' = Reduction.infer_conv ~typed:true Reduction.CONV env' (Environ.universes env') c c' in
	        cst'
	  in
