...
let warn_bad_relevance =
  CWarnings.create ~name:"bad-relevance" ~category:"debug" ~default:CWarnings.Disabled
    Pp.(fun x -> str "Bad relevance for binder " ++ Name.print x.binder_name ++ str ".")
...
let rec execute env cstr =
  ...
  match kind cstr with
    ...
    | Case (ci,p,is,c,lf) ->
        ...
        let is' = Option.Smart.map (fun is -> fst (execute_is_type env is)) is in
        let cstr = if c == c' && p == p' && lf == lf' && is == is' then cstr
          else mkCase(ci,p',is',c',lf')
        in
        cstr, type_of_case env ci p' pt is' c' ct lf' lft
...
let infer_local_decl env id = function
  | Entries.LocalDefEntry c ->
    let c, t = execute env c in
    let r = Retypeops.relevance_of_term env c in
    let x = make_annot (Name id) r in
    RelDecl.LocalDef (x, c, t)
  | Entries.LocalAssumEntry c ->
    let c, s = execute_is_type env c in
    let r = Sorts.relevance_of_sort s in
    let x = make_annot (Name id) r in
    RelDecl.LocalAssum (x, c)
