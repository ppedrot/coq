...
let constraints_of_universes g =
  ...
  in
  LMap.fold constraints_of g.entries Constraint.empty
...
let sort_universes g =
  ...
  let lowest_levels =
    LMap.mapi (fun u _ -> if Level.is_small u then 0 else 2)
      (LMap.filter
  ...
  let g =
    LMap.fold (fun u lvl g -> enforce_univ_eq u (types.(lvl)) g)
      lowest_levels g
  in
...
let pr_universes prl g =
  let graph = LMap.fold (fun u a l -> (u,a)::l) g.entries [] in
