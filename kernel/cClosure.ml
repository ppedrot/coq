...
let push_relevance infos x = { infos with i_relevances = x.binder_relevance :: infos.i_relevances }
...
let conv : (fconstr infos -> fconstr infos_tab -> fconstr -> fconstr -> bool) ref
  = ref (fun _ _ _ -> assert false)

let set_conv f = conv := f

let check_eqn info tab e (c,f) =
  let fc = mk_clos e c in
  if not (!conv info tab fc f) then raise InvertFail

let check_eqns info tab args eqns =
  (** FIXME incorrect **)
  let e = subs_cons (args, subs_id 0) in
  List.iter (check_eqn info tab e) eqns
...
and try_match_ctor_infos info tab ci params is =
  let open Declarations in function
    ...
    | Some { ctor_arg_infos = ainfos; ctor_out_tree = Some trees } ->
      begin try
          ...
          let () = check_eqns info tab (Array.append params args) !eqns in
          Some args
...
and case_inversion info tab ci (u,p,is) c v =
  ...
  let rec loop i =
    if Int.equal i nctors then None
      ...
      match try_match_ctor_infos info tab ci p is mip.mind_lc_info.(i) with
      | Some res ->
        Some (v.(i), res)
      | None -> loop (i+1)
  in
...
and norm_head info tab m =
  if is_val m then (incr prune; term_of_fconstr m) else
    match m.term with
      ...
      | FProd(na,dom,rng) ->
          mkProd(na, kl info tab dom, kl (push_relevance info na) tab rng)
      | FCoFix((n,(na,tys,bds)),e) ->
          let ftys = Array.Fun1.map mk_clos e tys in
          let fbds =
            Array.Fun1.map mk_clos (subs_liftn (Array.length na) e) bds in
          let infobd = Array.fold_left push_relevance info na in
