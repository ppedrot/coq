...
type 'a extended_conversion_function =
  ?l2r:bool -> ?reds:Conv_oracle.oracle -> env ->
...
(* Profiling *)
let gen_conv cv_pb ?(l2r=false) ?(reds=Conv_oracle.empty) env ?(evars=(fun _->None), universes env) =
...
let infer_conv ?(l2r=false) ?(evars=fun _ -> None) ?(ts=Conv_oracle.empty)
    env univs t1 t2 = 
  infer_conv_universes CONV l2r evars ts env univs t1 t2

let infer_conv_leq ?(l2r=false) ?(evars=fun _ -> None) ?(ts=Conv_oracle.empty)
    env univs t1 t2 = 
