...
let is_conv ?(reds=Conv_oracle.empty) env sigma = test_trans_conversion f_conv reds env sigma
let is_conv_leq ?(reds=Conv_oracle.empty) env sigma = test_trans_conversion f_conv_leq reds env sigma
let is_fconv ?(reds=Conv_oracle.empty) = function
  | Reduction.CONV -> is_conv ~reds
  | Reduction.CUMUL -> is_conv_leq ~reds

let check_conv ?(pb=Reduction.CUMUL) ?(ts=Conv_oracle.empty) env sigma x y =
  let f = match pb with
...
let infer_conv_gen conv_fun ?(catch_incon=true) ?(pb=Reduction.CUMUL)
    ?(ts=Conv_oracle.empty) env sigma x y =
  (** FIXME *)
  try
    let b, sigma = 
      let ans =
...
let vm_infer_conv = ref (infer_conv ~catch_incon:true ~ts:Conv_oracle.empty)
